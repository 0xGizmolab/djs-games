...
class ConnectFour {
    ...
    startGame (msg) {
        ...
        if(!oppenent) return msg.channel.send(`**Who do you wanna play Connect Four with? (You have to mention the person after the command.)**`)
        ...
        .setTitle(`🔴 It's ${msg.author.username}'s turn!`)
        ...
        msg.channel.send(initial).then(gameMessage => {
            ...
            gameCollector.on("collect", (reaction, user) => {
                ...
                if(user.id === gameData[player].member.id) {
                    ...
                    .setTitle(`${gameData[player].playerColor} - It's ${gameData[player].member.username}'s turn!`)
                    .setDescription(renderBoard(board))
                    .setFooter(`${challenger.username} vs ${oppenent.username}`)
